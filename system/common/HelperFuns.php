...
class HelperFuns
{
    ...
    static function downFile($_url, $_path = null, $_name = null, $_chmod = '0444')
    {
        ...
        $img = ob_get_contents();
        ob_end_clean();
        $exname = static::getFileExeName($_url);
        if (!$_name) {
            $_name = date('YmdHms') . static::randString(5) . '.' . $exname;
        } else {
            $_name = $_name . '.' . $exname;
        }
        //默认路径
    ...
    {
        //日志路径
        $filePath = ROOT_PATH . '/web/uploads/log';
        //创建目录
    ...
    static function autoCharset($string, $from = 'gbk', $to = 'utf-8')
    {
        ...
        if (is_string($string)) {
            ...
            foreach ($string as $key => $val) {
                $_key = static::autoCharset($key, $from, $to);
                $string[$_key] = static::autoCharset($val, $from, $to);
                if ($key != $_key)
                    unset($string[$key]);
            }
            return $string;
